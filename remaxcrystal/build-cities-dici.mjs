...
const CITIES = loadWindowData(path.join(ROOT, 'data/cities-dici.js'), 'CITIES_DICI');
const TAXES  = loadWindowData(path.join(ROOT, 'data/tax-data.js'), 'TAX_DATA');
const OUT_DIR = path.join(ROOT, '..', 'remaxdici');
...
function buildSeoSchema(city) {
  const baseUrl = 'https://agence.remaxdici.com';
  ...
  const realEstateAgent = {
    ...
    url: `${baseUrl}/guide-${city.fileSlug}.html`
  };
  const breadcrumb = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
  ...
  const place = {
    ...
    name: city.name,
    address: { '@type':'PostalAddress', addressLocality: city.name, addressRegion: 'QC', addressCountry: 'CA' },
    containedInPlace: { '@type':'AdministrativeArea', name: 'Lanaudière, Québec' },
    url: `${baseUrl}/guide-${city.fileSlug}.html`,
...
<link rel="canonical" href="https://agence.remaxdici.com/guide-${city.fileSlug}.html">
${buildSeoSchema(city)}`
  );
...
${buildQualityCards(city.qualityCards)}
  ...
  html = html.replace(
    ...
    `href="https://www.centris.ca/fr/outils/statistiques-immobilieres/${city.centrisRegionSlug}/${city.centrisSlug}"`
  );
  html = html.replace(
...
${buildTaxComparativeRows(city)}
  ...
  // Footer guides — 12 villes RE/MAX D'ici
  const footerGuides = `          <a href="guide-dici-terrebonne.html">Vivre à Terrebonne</a>
          <a href="guide-dici-mascouche.html">Vivre à Mascouche</a>
          <a href="guide-dici-repentigny.html">Vivre à Repentigny</a>
          <a href="guide-dici-lassomption.html">Vivre à L'Assomption</a>
          <a href="guide-dici-lavaltrie.html">Vivre à Lavaltrie</a>
          <a href="guide-dici-saint-lin-laurentides.html">Vivre à Saint-Lin–Laurentides</a>
          <a href="guide-dici-joliette.html">Vivre à Joliette</a>
          <a href="guide-dici-charlemagne.html">Vivre à Charlemagne</a>`;
  html = html.replace(
  ...
  // ============================================================
  //  MARQUE RE/MAX D'ici — logos, libellés, méga-menu, footer
  // ============================================================

  // Liens corporatifs Crystal -> D'ici (nav + footer : accueil, articles,
  // contact, carrière, courtiers, propriétés — URLs vérifiées sur remaxdici.com)
  html = html.replace(/https:\/\/remaxcrystal\.com\/fr/g, 'https://remaxdici.com/fr');

  // Logos Crystal -> D'ici (topbar + footer)
  html = html.replace(/brand_assets\/Crystal-long-blanc-crystal\.png/g, 'brand_assets/diciblanc.png');
  html = html.replace(/brand_assets\/Crystal-long-noir-crystal\.png/g, 'brand_assets/dicinoir.png');
  html = html.replace(/alt="RE\/MAX Crystal"/g, 'alt="RE/MAX D\'ici"');

  // Libellé topbar
  html = html.replace(
    /<div class="topbar-label">Guide de ville<\/div>/,
    `<div class="topbar-label">Guide de ville · D'ici</div>`
  );

  // Retire le ballon (montgolfière) du header — on garde seulement le logo D'ici
  html = html.replace(
    /\s*<img src="brand_assets\/digital_ballon\.png"[^>]*class="topbar-ballon">/,
    ''
  );

  // Retire le script courtiers.js (données propres à Crystal)
  html = html.replace(/\s*<script src="courtiers\.js"><\/script>/, '');

  // Méga-menu : 12 villes de Lanaudière
  const mmGrid = `      <span class="mm-cat">RE/MAX D'ici &middot; Lanaudière</span>
      <a href="guide-dici-terrebonne.html">Terrebonne</a>
      <a href="guide-dici-mascouche.html">Mascouche</a>
      <a href="guide-dici-repentigny.html">Repentigny</a>
      <a href="guide-dici-charlemagne.html">Charlemagne</a>
      <a href="guide-dici-lassomption.html">L'Assomption</a>
      <a href="guide-dici-lepiphanie.html">L'Épiphanie</a>
      <a href="guide-dici-saint-sulpice.html">Saint-Sulpice</a>
      <a href="guide-dici-lavaltrie.html">Lavaltrie</a>
      <a href="guide-dici-saint-roch-de-lachigan.html">Saint-Roch-de-l'Achigan</a>
      <a href="guide-dici-saint-lin-laurentides.html">Saint-Lin–Laurentides</a>
      <a href="guide-dici-saint-jacques.html">Saint-Jacques</a>
      <a href="guide-dici-joliette.html">Joliette</a>`;
  html = html.replace(
    /<span class="mm-cat">RE\/MAX Crystal[\s\S]*?<a href="guide-stesophie\.html">Sainte-Sophie<\/a>/,
    mmGrid
  );
  html = html.replace(
    /<span>15 villes &middot; Basses-Laurentides &amp; Rive-Nord<\/span>/,
    `<span>12 villes &middot; Lanaudière</span>`
  );

  // Copyright bas de page (le gabarit utilise un tiret, pas un point)
  html = html.replace(
    /&copy; 2026 RE\/MAX Crystal — Franchisé/,
    "&copy; 2026 RE/MAX D'ici — Franchisé"
  );

  // Retire le schéma LD+JSON résiduel du gabarit (RE/MAX Crystal · Blainville).
  // buildSeoSchema injecte déjà un RealEstateAgent à jour dans le <head>.
  html = html.replace(
    /<script type="application\/ld\+json">\{"@context":"https:\/\/schema.org","@type":"RealEstateAgent","name":"RE\/MAX Crystal"[\s\S]*?<\/script>\s*/,
    ''
  );

  // Adresse footer : l'agence Crystal de Sainte-Thérèse n'est pas l'adresse D'ici.
  // On remplace par une mention régionale (pas d'adresse/téléphone inventés).
  html = html.replace(
    /<address class="footer-address">[\s\S]*?<\/address>/,
    `<address class="footer-address">
            Au service de toute la région de<br>
            Lanaudière, Québec<br>
            <a href="https://agence.remaxdici.com" target="_blank" rel="noopener">agence.remaxdici.com</a>
          </address>`
  );
...
for (const [slug, city] of Object.entries(CITIES)) {
  try {
    ...
    const out = path.join(OUT_DIR, `guide-${city.fileSlug}.html`);
    fs.writeFileSync(out, html, 'utf8');
    const size = fs.statSync(out).size;
    sizes.push(size);
    console.log(`[OK] guide-${city.fileSlug}.html (${(size/1024).toFixed(1)} KB)`);
    ok++;
  } catch (e) {
